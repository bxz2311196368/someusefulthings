						...
						if (nbt != null && type != null)
						{
							...
							case "Crafter":
							{
								...
								if (te != null && te instanceof CraftingTableTileEntity)
								{
									...
									t.readFromNBT((NBTTagCompound) nbt.getTag("Full_Data"));
									if (t.isLockRecipe())
										t.clinetSetCache();
								}
								break;
						...
						if (nbt != null && type != null)
						{
							...
							case "Crafter":
								if (nbt.hasKey("world") && nbt.hasKey("BlockPos"))
								{
									...
									boolean lock = nbt.getBoolean("Lock");
									TileEntity te = world.getTileEntity(pos);
									if (te != null && te instanceof CraftingTableTileEntity)
									{
										CraftingTableTileEntity t = (CraftingTableTileEntity) te;
										if (nbt.hasKey("Need_Update") && nbt.getBoolean("Need_Update"))
										{
											t.syncData(Server.getPlayerList()
													.getPlayerByUsername(nbt.getString("Player")));
											break;
										}
										if (nbt.hasKey("Button") && nbt.getBoolean("Button"))
										{
											t.setAuto(auto);
											t.setStayInSlot(output);
											if (lock && !t.isLockRecipe())
												t.LockRecipe();
											else
												t.setLockRecipe(lock);
										}
									}
								}
								break;
							case "TP":
								if (nbt.hasKey("Player") && nbt.hasKey("Button") && nbt.hasKey("Select"))
								{
									...
									int button = nbt.getInteger("Button"), select = nbt.getInteger("Select"),
									...
									case 25:
										if (nbt.hasKey("new") && nbt.getBoolean("new") && nbt.hasKey("PosName"))
										{
											cp.addPos(new BlockPosWithDimAndName(
													...
													player.dimension, nbt.getString("PosName")));
										}
										break;
									case 26:
									...
									TPLocationSync message = new TPLocationSync();
									IStorage<ITPLocation> storage = CapabilityLoader.TP_MENU.getStorage();
									message.nbt = (NBTTagCompound) storage.writeNBT(CapabilityLoader.TP_MENU, cp, null);
									NetworkLoader.instance.sendTo(message, player);
								}
							case "XPReservoir":
								if (nbt.hasKey("Player") && nbt.hasKey("Button") && nbt.hasKey("Dim")
										&& nbt.hasKey("BlockPos"))
								{
									EntityPlayer player = Server.getPlayerList()
											.getPlayerByUsername(nbt.getString("Player"));
